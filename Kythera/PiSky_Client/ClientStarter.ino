 ...
 */
 ...
 #define DATA_ARRAY_SIZE_BYTES 35
 #define MESSAGE_ARRAY_SIZE_BYTES 102
 #define DATA_ARRAY_SIZE_BITS 280
 ...
 */
 ...
 struct CAN_filter_t clientMask;
 clientMask.rtr = 0;
 clientMask.ext = 0;
 ...
 typedef struct data
 {
  ...
  float roll;          /* current vehicle roll */
  ...
  float pressure;      /* current interior pressure */
  float temp;          /* current interior pressure */
  ...
  uint16_t  flight_time;    /* miliseconds elapsed since launch */
} data;
...
typedef struct message
{
  ...
  bool      health;        /* user specified health bit */
 ...
 */
 void updateData()
 {
   ...
   if(index++ != DATA_ARRAY_SIZE_BITS){
     error("full data message not recieved...only recieved " + index + "bits");
     recieved = false;
   } else{
     recieved = true;
   }
 ...
 float getRoll()
 {
   if(recieved) return currentData.roll;
   return -1;
 }
